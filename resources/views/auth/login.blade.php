<x-auth-layout title="Login">
    <x-slot name="header">Selamat Datang Kembali</x-slot>

    <!-- Alert Messages -->
    @if (session('success'))
        <div class="mb-4 p-4 rounded-xl bg-green-500/20 border border-green-500/30 text-green-400">
    ...
    <form action="{{ route('login.post') }}" method="POST" class="space-y-5">
        ...
        <div class="flex items-center justify-between text-sm">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="remember" class="w-4 h-4 rounded border-white/20 bg-white/5 text-purple-500 focus:ring-purple-500 focus:ring-offset-0">
                <span class="text-gray-400">Ingat saya</span>
            </label>
            <a href="#" class="text-purple-400 hover:text-purple-300 transition-colors">Lupa password?</a>
        </div>
